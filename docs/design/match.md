...
`POST /v1/ingest` accepts `multipart/form-data` field `file` or a raw body. `202` returns `{"session","jobs"}` with created rows as `pending`; the worker matches them. Missing `title` or an empty payload is `400`. Jobs persist at `{data_dir}/jobs-{session}.json` until `session.ttl_ms` (clamped to `session.ttl_max_ms`) or `DELETE /v1/jobs?session=`.
...
`POST /v1/scan` with `{ "path": "...", "mode": "rescan"|"changes", "require_episode_nfo": true }` lists video files under a directory inside `browse_root` and returns `202 {"session","files","mode"}` immediately. Omit `mode` or send `"rescan"` for a full provider search. `"changes"` groups the **current** tree the same way, then splits titles by sidecar completeness (title NFO + card poster). `require_episode_nfo` is Changes-only (`*bool`): omit or `true` (default) also requires `episode.nfo` or `<basename>.nfo` on every owned video; `false` skips that. Rescan ignores the field. Unknown `mode` is `400`. Non-empty `path` must be absolute and under `browse_root`; empty path still means `browse_root`. Zero videos is still `400`.
